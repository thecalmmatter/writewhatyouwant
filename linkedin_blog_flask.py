...
editor = Agent(
    role="Editor",
    goal="Edit the New York Times post to align with journalistic standards and add some interesting icons and the brand’s voice.",
    backstory=(
        "As an editor, you refine the New York Times post for clarity, grammar, structure and balance. "
...
hook_generator = Agent(
    role="Hook Generator",
    goal="Add a New York Times post hook.",
    backstory=(
...
plan = Task(
    description=(
        "1. Deep research and prioritize the latest trends, key players, and noteworthy news on {topic}.\n"
...
write = Task(
    ...
    expected_output="A well-structured, engaging New York Times  post in markdown format, ready for publication.",
    agent=writer
)

edit = Task(
    description="Proofread and refine the New York Times post for clarity, grammar, structure, and tone consistency.",
    expected_output="A polished New York Times post in markdown format, ready for publication.",
    agent=editor
)

hook = Task(
    description="Craft an engaging, quirky, and compelling hook for a New York Times post that grabs attention instantly. The hook should spark curiosity, encourage engagement, and set the stage for the main content.",
    expected_output="A sharp, well-structured New York Times post hook in markdown format, ensuring clarity, grammatical accuracy, and an engaging tone, ready for publication.",
